...
2. **Unused scraping code: handed to task 002.** The icon download was removed, then restored at the user's request: the icon stays out of the bulletin because it's holy and mustn't be thrown away, but it may be printed separately someday. It is now fetched through the archive and only warns on failure. The rest (DCS readings and prokeimenon, saint/feast hymns, `EpistlePageData.mode`) is left for task 002 to wire in or delete.
...
6. **Smaller items: done.** `yaml_io.py` replaces `aiofiles` and the duplicate `write_yaml`, the scrapes run concurrently, and the README has usage. `run_date + 6` is intended: the mission's Saturday services use the following Sunday's liturgy (now in AGENTS.md, "Parish context").
